...
DJANGO: RE-CREATING THE PROJECT DATABASE
    ...
    >> If you need to re-create your db from the beginning, do it:
        ...
        1) Delete the current database;
            ...
            >> PostgreSQL:
                /database/PostgreSQL/deleting-db.txt
        ...
        2) Create the new database:
            ...
            >> PostgreSQL:
                /database/PostgreSQL/creating-db-locally.txt
        ...
        4) In your Django, through each sub-app migration-folder, delete all its content (except the dunder init). Check if you are visualizing the __pycache__ folder. It should be deleted too, but the project settings could be hidden them!

        5) Edit your .env file as needed (the new db name, for example);

        6) CRITICAL:
            close all Terminals/Prompts you ran with the old .env before (it avoids issues with old environment variables)!

        7) Run 'makemigrations' for each sub-app, and then run 'migrate';
        
        8) (If applicable)
            If you'll use the CMS, run 'createsuperuser' command;
        ...
        10) Run 'runserver' to test Django!
...
HOW TO RE-INSTALL ONLY THE DJANGO:
    .../django/1-install-and-first-steps/1-reinstall-new-copy.md
