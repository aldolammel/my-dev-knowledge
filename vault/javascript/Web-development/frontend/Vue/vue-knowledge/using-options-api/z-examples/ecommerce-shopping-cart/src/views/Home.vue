<template>
  <div class="home">
    <div class="products">
      ...
      >
        <div class="product-image" :style="{ backgroundImage: `url(${prod.image})` }" />
        <h4>{{ prod.title }}</h4>
        <p class="price">US$ {{ prod.price.toFixed(2) }}</p>
        <button v-if="!isInCart(prod)" type="button" @click="btCartAddProd(prod)">
          Add to Cart
        </button>
        <button v-else class="remove" @click="btCartDelProd(prod.id)">Remove from cart</button>
      </div>
    </div>
  </div>
</template>
