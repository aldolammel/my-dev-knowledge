  ...
  <div class="cart">
    <div class="items">
      <template v-if="cart.length">
        ...
        <div v-for="(prod, idx) in cart" :key="idx" class="item">
          <div class="remove" @click="btCartDelProd(prod.id)">Remove item</div>
          <div class="photo">
          ...
          <div class="price">
            <span class="quantity-area">
              <button :disabled="prod.quantity <= 1" type="button" @click="prod.quantity--">
                -
              </button>
              <span class="quantity">{{ prod.quantity }}</span>
              <button type="button" @click="prod.quantity++">+</button>
            </span>
            <span class="amount">US$ {{ (prod.price * prod.quantity).toFixed(2) }}</span>
          </div>
        </div>
        <!-- EACH CART PRODUCT - END -->
        <div class="grand-total">Total: US$ {{ calcCartTotal() }}</div>
      </template>
      <template v-else>
