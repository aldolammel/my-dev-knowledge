"""

MODULE AND PACKAGE (Módulos e Pacotes):
Packages on Python is the same of libraries in other languages.

"""

import class22_fnc                            # importing my functions from another file.

# MY COLORS
_corT = '\033[1;32m'
_corF = '\033[1;31m'
_corOut = '\033[m'

# Pre-lessons:

_userNum = int(input('Type an integer: '))
_factorial = class22_fnc.fnc_factorial(_userNum)
print(
    f'The factorial of {_userNum} is: {_factorial}'
    f'\nThe double of {_userNum} is: {class22_fnc.fnc_double(_userNum)}'
    f'\nThe triple of {_userNum} is: {class22_fnc.fnc_triple(_userNum)}'
)

# ----------------------------------------
# LESSON 107 + 108 + 109:

print('\n\nLesson 107+108+109 >> Build a program xxxxxxxxxxxxxx:\n')

_userPrice = float(input('Type a price: R$'))

class22_fnc.fnc_resume(_userPrice, 30)
