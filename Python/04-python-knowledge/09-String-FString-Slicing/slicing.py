...
print(
    f'At first, easy to see the phrase got {len(_phrase)} characters.',
    f'However let\'s go through it deeper:\n',
)

print('So think: ', _phrase[11:])  # using slicing from 11th to the last phrase char.
...
print(_phrase[-9:])  # from the last til 9th char (not index), taking 'nowadays?' (reverse).
...
print(
    f'Your phrase has {_characterCounter} characters, {_characterCounter - _spaceCounter} letters',
...
print(
    f'{_userName} converted to lowercase is \'{_userName.lower()}\' and uppercase is \'{_userName.upper()}\'. \n'
...
_unity = (
    _givenNumb // 1 % 10
)  # número com divisão-inteira (//) por 1 (ele mesmo), divido por 10 (% = resto da divisão)
_ten = _givenNumb // 10 % 10
_hundred = _givenNumb // 100 % 10
_thousand = _givenNumb // 1000 % 10

print(
    f'Analysing the number {_givenNumb}: \n'
    f'Unity = {_unity}\n'
    f'Ten = {_ten}\n'
    f'Hundred = {_hundred}\n'
...
_comparison = _city[:5] == str(
    'SANTO'
)  # I'm counting the digits from first until 4th digit in _city string.
...
print(
    f'It finds {_aCounted} letter \'A\' in your phrase. The first \'A\' occurs in the character position number'
...
print(
    f'The full name \'{_fullName27}\' is composed by the first word \'{_firstWord27}\''
