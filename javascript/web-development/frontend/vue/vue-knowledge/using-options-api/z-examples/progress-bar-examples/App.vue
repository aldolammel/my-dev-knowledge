...
export default {
  ...
  data() {
    return {
      ...
      currentStep: 0,
      steps: ['Personal Info', 'Contact Details', 'Preferences', 'Review', 'Complete'],
      
      // Circular progress value
      circularProgress: 75,
      
      // SVG circle properties for circular progress
      circumference: 2 * Math.PI * 50 // 2πr where r=50
    }
  },
  ...
  computed: {
    ...
    circularOffset() {
      const progress = this.circularProgress / 100;
      return this.circumference * (1 - progress);
    }
  },
  
  methods: {
    // Update basic progress with random value
    updateBasicProgress() {
      this.basicProgress = Math.floor(Math.random() * 101);
    },
    
    // Increase custom progress by 10%
    increaseCustomProgress() {
      if (this.customProgress < 100) {
        this.customProgress = Math.min(this.customProgress + 10, 100);
      }
    },
    
    // Decrease custom progress by 10%
    decreaseCustomProgress() {
      if (this.customProgress > 0) {
        this.customProgress = Math.max(this.customProgress - 10, 0);
      }
    },
    
    // Reset custom progress to 0
    resetCustomProgress() {
    ...
    // Move to next step in multi-step progress
    nextStep() {
    ...
    // Move to previous step in multi-step progress
    previousStep() {
    ...
    // Reset multi-step progress to beginning
    resetSteps() {
      this.currentStep = 0;
    },
    
    // Increase circular progress by 5%
    increaseCircularProgress() {
      if (this.circularProgress < 100) {
        this.circularProgress = Math.min(this.circularProgress + 5, 100);
      }
    },
    
    // Decrease circular progress by 5%
    decreaseCircularProgress() {
      if (this.circularProgress > 0) {
        this.circularProgress = Math.max(this.circularProgress - 5, 0);
      }
    }
  },
